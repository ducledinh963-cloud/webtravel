<div class="admin-form-container">
    ...
    <form action="index.php?act=addgt" method="POST" enctype="multipart/form-data">
        <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 20px;">
            <div class="admin-form-group">
                <label for="name">Tên hoạt động giải trí / Khu vui chơi *</label>
                <input type="text" id="name" name="name" required placeholder="Ví dụ: Cáp Treo Hòn Thơm - Phú Quốc..." value="<?php echo isset($_POST['name']) ? htmlspecialchars($_POST['name']) : ''; ?>">
            </div>
        ...
        <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 20px;">
            <div class="admin-form-group">
                ...
                <input type="text" id="address" name="address" required placeholder="Ví dụ: Phường An Thới, Phú Quốc, Kiên Giang..." value="<?php echo isset($_POST['address']) ? htmlspecialchars($_POST['address']) : ''; ?>">
            </div>

            <div class="admin-form-group">
                <label for="price">Giá vé / phí dịch vụ trung bình (VNĐ) *</label>
                <input type="number" id="price" name="price" required min="0" placeholder="Ví dụ: 450000" value="<?php echo isset($_POST['price']) ? htmlspecialchars($_POST['price']) : ''; ?>">
            </div>
        </div>

        <div class="admin-form-group">
            <label for="image">Hình ảnh đại diện hoạt động</label>
            ...
            <div style="margin-top: 8px; font-size: 12px; color: var(--text-muted);">
                Nếu không chọn tệp, hệ thống sẽ sử dụng hình ảnh mẫu mặc định (`uploads/entertainment1.png`).
            </div>
        </div>

        <div class="admin-form-group">
            <label for="description">Mô tả chi tiết hoạt động giải trí</label>
            <textarea id="description" name="description" rows="6" placeholder="Nhập giới thiệu chi tiết, thông tin giờ mở cửa, lưu ý đặc biệt..." style="width: 100%; font-family: inherit; line-height: 1.5; resize: vertical;"><?php echo isset($_POST['description']) ? htmlspecialchars($_POST['description']) : ''; ?></textarea>
        </div>
