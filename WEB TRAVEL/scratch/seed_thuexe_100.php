...
// Danh sách 100 xe du lịch thực tế đầy đủ
$cars = [
    // 1 - 20
...
// Cập nhật database.sql bằng cách thay thế phần thuexe cũ
$db_sql_path = __DIR__ . '/../database.sql';
if (file_exists($db_sql_path)) {
    $db_sql = file_get_contents($db_sql_path);
    
    // Tìm vị trí của phần định nghĩa thuexe cũ
    $pos = strpos($db_sql, 'DROP TABLE IF EXISTS `thuexe`;');
    if ($pos === false) {
        $pos = strpos($db_sql, '-- Cấu trúc bảng cho bảng `thuexe`');
    }
    
    if ($pos !== false) {
        // Cắt bỏ phần cũ từ vị trí tìm thấy
        $db_sql = substr($db_sql, 0, $pos);
    }
    
    // Ghép phần mới vào
    $append_sql = "-- Cấu trúc bảng cho bảng `thuexe`\n--\n" . $sql_schema . "\n-- Thêm dữ liệu cho bảng `thuexe`\n" . $sql_inserts;
    file_put_contents($db_sql_path, rtrim($db_sql) . "\n\n" . $append_sql);
    echo "database.sql updated and cleaned up with 100-car thuexe table structure and seeds!\n";
} else {
    echo "database.sql file not found at $db_sql_path\n";
}

// Chạy SQL trong database hiện tại bằng cách kết nối trực tiếp đến 127.0.0.1 (để khắc phục lỗi IPv6 localhost)
try {
    ...
    $pdo->exec($sql_schema);
    echo "Seeding 100 car records into thuexe table...\n";
    $pdo->exec($sql_inserts);
    echo "Database table seeded successfully with 100 cars via CLI!\n";
} catch (Exception $e) {
